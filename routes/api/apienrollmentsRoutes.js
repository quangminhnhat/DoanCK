...
router.get(
  ...
  `;
    ...
    sql.query(connectionString, query, (err, rows) => {
      ...
      res.json({ enrollments: rows, user: req.user });
    });
  }
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ success: true, redirect: "/enrollments" });
    } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ enrollment: enrollment[0], students, classes, user: req.user });
    } catch (err) {
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      // 1. Validate input
      if (!class_id) {
        return res.status(400).json({ error: "Class must be selected." });
      }
      ...
      if (!newClass) {
        return res.status(404).json({ error: "The selected class does not exist." });
      }

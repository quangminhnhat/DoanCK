...
router.get("/course-detail/:id", async (req, res) => {
  try {
    ...
    if (!course) {
      return res.status(404).json({ message: "Course not found.", user: req.user });
    }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ courses: courses, student: studentInfo[0], user: req.user });
    } catch (err) {
      console.error("Error fetching available courses:", err);
      res.status(500).json({ error: "Error loading available courses" });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    `;
      ...
      res.json({ success: true, redirect: "/my-courses" });
    } catch (err) {
...
router.get("/my-courses", checkAuthenticated, async (req, res) => {
  try {
    ...
    res.json({ user: req.user, courses: processedCourses, messages: { error: req.flash("error"), success: req.flash("success") } });
  } catch (error) {

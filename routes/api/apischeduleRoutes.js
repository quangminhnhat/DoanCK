...
router.get(
  ...
  async (req, res) => {
    ...
    `;
      ...
      res.json({ schedules, user: req.user });
    } catch (err) {
  ...
  async (req, res) => {
    ...
    `;
      ...
      if (hasCourseCol) {
        ...
        if (!courseResult.length) return res.status(404).json({ error: 'Class not found' });
...
router.get("/schedules/:id/edit", checkAuthenticated, authenticateRole("admin"), async (req, res) => {
  try {
    ...
    if (!scheduleResults.length) {
      ...
      return res.status(404).json({ message: 'Schedule not found', user: req.user });
    }
    ...
    res.status(500).json({
      error: 'Error loading schedule edit form',
      detail: String(err),
      user: req.user
    });
  }

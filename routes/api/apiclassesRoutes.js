...
router.post(
  ...
  async (req, res) => {
    ...
    `;
      ...
      res.json({ success: true, redirect: "/classes" });
    } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ user: req.user, classItem, courses: courseList, teachers: teacherList });
    } catch (error) {
      console.error("Error loading class edit form:", error);
      res.status(500).json({ error: "Error loading class edit form" });
    }
  }
);
  ...
  async (req, res) => {
    ...
    `;
      ...
      if (teacherConflicts.length > 0) {
        return res.status(409).json({ error: `Schedule Conflict: The selected teacher is already assigned to another class during the chosen time and day. Conflicting class: ${teacherConflicts[0].class_name}` });
      }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Return courses and teachers data
      res.json({
        user: req.user,
        courses: courses,
        teachers: teachers,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ user: req.user, classInfo: classInfo[0], students: students });
    } catch (error) {
